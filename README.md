...
| **A permission layer** | Six ordered gates over every tool call: secret paths, catastrophic bash, destructive git, privileged commands, agent routing, and a headless bash allowlist that fails closed |
...
| **Sub-agents, teammates, jobs, worktrees** | Depth-limited dispatch with per-provider concurrency, long-lived named teammates with a delivery obligation welded into the spawn path, a cross-session background job directory, and `isolation: worktree` that never nests and never `rm -rf`s a dirty tree |
