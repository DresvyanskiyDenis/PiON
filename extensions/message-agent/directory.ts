...
/**
 * Takes everything currently in this session's inbox, oldest first, and removes it.
 *
...
export async function drainInbox(root: string, name: string): Promise<DrainResult> {
  ...
  for (const entry of entries.filter((e) => e.endsWith(".json")).sort()) {
    ...
    try {
      ...
      if (parsed.schema !== AGENT_SCHEMA || typeof parsed.message !== "string") {
        throw new MessageAgentError(
          `envelope ${entry} is schema ${String(parsed.schema)} with ${typeof parsed.message} message`,
        );
      }
