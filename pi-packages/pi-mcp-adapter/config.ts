...
// ---------------------------------------------------------------------------
// LOCAL PATCH — not upstream. Recorded in pi-packages/vendor.lock.json under `localPatches`,
// which states what it changes, why it cannot live outside the vendor boundary, and how to
// re-apply it on a version bump.
//
// Upstream reads `<cwd>/.mcp.json` and `<cwd>/.pi/mcp.json` unconditionally, at `session_start`,
// before Pi's project-trust decision is consulted — so cloning a repo and opening it is enough to
// let that repo name the MCP servers this agent talks to. `lifecycle: "eager" | "keep-alive"`
// servers are spawned during initialization, so a `tool_call` gate is too late; the read itself is
// the boundary and `getConfigSources` is the only funnel through which project sources enter.
//
// The gate is DEFAULT-DENY: with no gate installed, project sources are dropped. A missing gate
// means nobody answered the trust question, and "nobody answered" is not "yes". `extensions/
// trust.ts` installs the real one at register time (before any `session_start`, packages resolve
// first) and reconciles it against `ctx.isProjectTrusted()` once a context exists.
// ---------------------------------------------------------------------------

/** Returns true when the project rooted at `cwd` may contribute MCP config. */
export type ProjectConfigTrustGate = (cwd: string) => boolean;

let projectConfigTrustGate: ProjectConfigTrustGate | undefined;
let projectConfigTrustLog: ((message: string) => void) | undefined;
const deniedProjectRoots = new Set<string>();

/**
 * Install the trust gate. Pass `undefined` to uninstall — which restores default-deny, never
 * upstream's default-allow.
 */
export function setProjectConfigTrustGate(
  gate: ProjectConfigTrustGate | undefined,
  log?: (message: string) => void,
): void {
  projectConfigTrustGate = gate;
  projectConfigTrustLog = log;
  deniedProjectRoots.clear();
}

/** What the gate currently says about `cwd`, without the logging side effect. */
export function isProjectConfigTrusted(cwd = process.cwd()): boolean {
  if (projectConfigTrustGate === undefined) return false;
  try {
    return projectConfigTrustGate(cwd) === true;
  } catch (error) {
    // A throwing gate is a bug in the gate, and a bug in the gate is not consent.
    projectConfigTrustLog?.(
      `mcp: project-config trust gate threw for ${cwd} — denying project MCP config: ${error instanceof Error ? `${error.name}: ${error.message}` : String(error)}`,
    );
    return false;
  }
}

function allowProjectConfigSources(cwd: string): boolean {
  if (isProjectConfigTrusted(cwd)) return true;
  if (!deniedProjectRoots.has(cwd)) {
    deniedProjectRoots.add(cwd);
    const why =
      projectConfigTrustGate === undefined
        ? "no trust gate is installed (extensions/trust.ts did not register)"
        : "this project's MCP config carries no matching approval record";
    projectConfigTrustLog?.(
      `mcp: ignoring ${PROJECT_CONFIG_NAME} and ${PROJECT_PI_CONFIG_NAME} under ${cwd} — ${why}. Project-defined MCP servers are NOT loaded.`,
    );
  }
  return false;
}
...
/**
 * Every server name a PROJECT-scoped source contributes for `cwd`.
 *
 * LOCAL PATCH, layer 2 of the same fix. `server-manager.ts` uses this to decide which stdio spawns
 * must be routed through `config/bin/mcp-stdio-guard`, so that "did the operator remember to opt
 * this server into the wrapper?" stops being the question — a project-sourced server never gets to
 * answer it for itself.
 *
 * A name that appears in BOTH a global and a project source counts as project-sourced: the project
 * copy can repoint `command`/`args`, so the safe reading of "contributed by a project" is "named by
 * a project at all". If the gate denied, `getConfigSources` returns no project sources and this is
 * empty — the two layers agree by construction rather than by a second policy.
 */
export function projectScopedServerNames(cwd = process.cwd(), overridePath?: string): Set<string> {
  const names = new Set<string>();
  for (const source of getConfigSources(overridePath, cwd)) {
    if (source.scope !== "project") continue;
    const loaded = readValidatedConfig(source.readPath, `MCP config from ${source.readPath}`);
    if (!loaded) continue;
    for (const name of Object.keys(loaded.mcpServers ?? {})) names.add(name);
  }
  return names;
}

function getConfigSources(overridePath?: string, cwd = process.cwd()): ConfigSourceSpec[] {
  const projectAllowed = allowProjectConfigSources(cwd);
  ...
  if (projectAllowed && projectPath !== userPath) {
    sources.push({
      id: "shared-project",
  ...
  if (projectAllowed && projectPiPath !== userPath && projectPiPath !== projectPath) {
    sources.push({
      id: "pi-project",
