...
// LOCAL PATCH (pi-config) — layer 2 of the project-MCP-config fix. See ./stdio-guard.ts.
import { guardProjectStdioSpawn, setStdioGuardLogSink } from "./stdio-guard.ts";
import {
...
export class McpServerManager {
  ...
  private async createConnection(
    ...
    if (definition.command) {
      ...
      const childEnv = resolveEnv(definition.env, name);
      // LOCAL PATCH (pi-config), layer 2 — a project-sourced stdio server is wrapped whether or not
      // its own entry asked to be. See guardProjectStdioSpawn().
      const guarded = guardProjectStdioSpawn(name, command, args, childEnv, this.defaultCwd);
...
// LOCAL PATCH (pi-config) — ./stdio-guard.ts cannot import ./logger.ts (parameter properties are
// rejected by Node's strip-only TypeScript loader, which would make the module untestable), so the
// adapter's logger is handed to it from here instead.
setStdioGuardLogSink((level, message) => {
